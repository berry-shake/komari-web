...
import { compareVersions, isNewerVersion, parseVersion, isMaintenanceVersion } from '../src/utils/version.ts';

test('fork revisions and upstream migrations are ordered numerically', () => {
  for (const [newer, older] of [
    ['1.3.2-fork.1', '1.3.2'], ['1.3.2-fork.2', 'v1.3.2-fork.1'],
    ['1.3.2-fork.10', '1.3.2-fork.9'], ['1.3.3-fork.1', '1.3.2-fork.99'],
  ]) {
...
test('development and invalid versions are excluded', () => {
  for (const value of ['dev', 'Snapshot-123', '1.3.2-beta.1', '1.3.2-fork.0', '1.3.2junk', '1.3', '01.3.2', null]) {
...
test('out-of-order GitHub release responses select the newest revision', () => {
  assert.deepEqual(['1.3.2-fork.2', '1.3.2-fork.10', '1.3.2-fork.1'].sort((a, b) => compareVersions(b, a)),
    ['1.3.2-fork.10', '1.3.2-fork.2', '1.3.2-fork.1']);
});

test('single database updates exclude retained newer baselines', () => {
  const tags = ['1.3.2-fork.100', '1.2.3-fork.2', '1.2.3-fork.10', '1.2.3', '1.2.3-fork.01', '1.2.5-fork.1'];
  assert.deepEqual(tags.filter(isMaintenanceVersion).sort((a,b) => compareVersions(b,a)), ['1.2.3-fork.10', '1.2.3-fork.2']);
});
