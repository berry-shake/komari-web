import { agentInstallerUrl } from "@/config/distribution";
...
export function ActionsCell({ row }: { row: Row<z.infer<typeof schema>> }) {
  ...
  const generateCommand = () => {
    ...
    switch (selectedPlatform) {
      case "linux":
        finalCommand =
          `wget -qO- ${agentInstallerUrl("install.sh")} | sudo bash -s -- ` +
          quoteShellArgs(args);
        break;
      case "windows":
        finalCommand =
          `powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ` +
          `"iwr '${agentInstallerUrl("install.ps1")}'` +
      ...
      case "macos":
        finalCommand =
          `bash <(curl -fsSL ${agentInstallerUrl("install.sh")}) ` +
          quoteShellArgs(args);
        break;
    }
    return finalCommand;
